class ElementInspector {
    async inspectElement(page, selector) {
        const result = await page.evaluate((sel) => {
            ...
            function detectVueComponent(node) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const vueInstance = node.__vueParentComponent;
                if (!vueInstance)
                    return null;
                const name = vueInstance.type?.name ??
    ...
    async getComponentTree(page, selector, maxDepth = 10) {
        const tree = await page.evaluate(({ sel, depth }) => {
            ...
            function getVueInfo(node) {
                ...
                if (!vue)
                    return null;
                const name = vue.type?.name ?? vue.type?.__name ?? vue.type?.displayName ?? 'UnknownVue';
                return {
                    name,
                    framework: 'vue',
